...
class EvenSized(Sized):
    pass

COLOR_SCALAR = Annotated[int, ValueRange(0, 255)]
POSITIVE_INTEGER = Annotated[int, ValueRange(0, float("inf"), include_min=False)]

RGB: TypeAlias = tuple[COLOR_SCALAR, COLOR_SCALAR, COLOR_SCALAR]
RGBA: TypeAlias = tuple[COLOR_SCALAR, COLOR_SCALAR, COLOR_SCALAR, COLOR_SCALAR]
FLATTENED_POINT_LIST = Annotated[list[int], EvenSized]
IMAGE_SIZE: TypeAlias = tuple[POSITIVE_INTEGER, POSITIVE_INTEGER]

#=============================================================================================================================#

EPSILON = 0.01

def adjust_alpha(inp: RGB | RGBA, alpha: COLOR_SCALAR) -> RGBA:
    if inp:
        return (inp[0], inp[1], inp[2], alpha)

def get_border(xy: FLATTENED_POINT_LIST, epsilon=1):
    min_x = float("inf")
    min_y = float("inf")
    max_x = float("-inf")
    max_y = float("-inf")
    for x, y in pairwise(xy):
        ...
        if y < min_y:
            ...
            max_y = y
    return (
        math.floor(min_x) - epsilon,
        math.floor(min_y) - epsilon,
        math.ceil(max_x) + epsilon,
        math.ceil(max_y) + epsilon
    )

#=============================================================================================================================#

class SSAAImageProcessing:
    fonts = {}
    ...
    def __init__(self, canvas: Image.Image, aa: int = 2):
        ...
